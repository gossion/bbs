...
func RebindForFlavor(query, flavor string) string {
	...
	case MSSQL:
		query = strings.Replace(query, "MEDIUMTEXT", "TEXT", -1)
        //Use BIT(1,0) to replace BOOL(true, false) for mssql.
		query = strings.Replace(query, "BOOL", "BIT", -1)
        query = strings.Replace(query, "true", "1", -1)
        query = strings.Replace(query, "false", "0", -1)
		return query
	default:
